...
> - **ライセンス**: 4 エージェントまでの構成であれば**完全無料（ライセンス不要）**でご利用いただけます。5 エージェント以上の大規模構成で運用する場合のみ、ライセンスキーが必要となります。
...
| `licenseKey` | `string` | - | 有効なライセンスキー（5エージェント以上の場合に必須）。 |
...
const network = new Network({
  ...
  licenseKey: process.env.SYNAPSE_LICENSE_KEY // 5エージェント以上で必須
});
